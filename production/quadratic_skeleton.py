...
def fold_180(polys, flap_size, eps=Fraction(-1, 2)):
    return fold(polys, Edge(Point(1 - Fraction(flap_size), Fraction(1)),
                            Point(1 - Fraction(flap_size), Fraction(0))))
def fold_180_2(polys, flap_size, eps=Fraction(-1, 95)):
    return fold(polys, Edge(Point(1 - 3 * Fraction(flap_size) + eps, Fraction(0)),
                            Point(1 - 3 * Fraction(flap_size) + eps, Fraction(1))))
...
def quadratic_skeleton(n):
    ...
    polys = fold_180_2(polys, flap_size)
